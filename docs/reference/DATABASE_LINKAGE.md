...
> **Storage note:** the system is self-hosted on **PostgreSQL** (migrated off Firestore). Each
> "collection" named below is a Postgres table holding JSON documents, reached only through the
> Node/Express API. The **relational linkage described here is unchanged** — only the storage
> engine and access path differ from the Firestore era.
...
An employee's profile is stored in the `users` collection (a PostgreSQL table of JSON documents) and is defined by the `User` interface (`src/types.ts`). It acts as the central hub of the system, linking out to other collections using unique identifier fields:
...
### B. Embedded Sub-object Arrays (JSON-serialized inside the document)
