...
12. [Authorization Rules Summary](#authorization-rules-summary)
...
EPROM CMS is a **React + TypeScript** single-page application, backed by a **self-hosted Node + Express + PostgreSQL API**, that manages employee competency development across a multi-project energy sector organization. (It was migrated off Firebase — see [`../migration/`](../migration/).) It covers:
...
| Backend (API) | Node 20 + Express (`server/`) — REST, JWT auth, authz middleware |
| Database | PostgreSQL — each collection is a table of JSON documents, reached **only** via the API |
| Auth | Email/password → JWT (HS256), bcrypt-hashed passwords, issued by the API |
| Deploy | Docker Compose: postgres + api + web (nginx) + backup |
| Icons | Lucide React |
| Charts | Recharts |
| Excel | xlsx (SheetJS) |
| UI Extras | clsx, tailwind-merge |

**Data access:** the SPA calls the self-hosted API at `VITE_API_URL` (`/api` behind nginx). `src/services/store.ts` uses Firestore-shaped calls served by compat shims (`firestore-compat.ts` + `auth-compat.ts`) over `api-client.ts`; updates arrive by **polling** (`VITE_POLL_INTERVAL_MS`), not `onSnapshot`.

---

## Project Structure

> **Note:** the tree below predates the 2026-07 restructure — the **frontend source now lives
> under `src/`** and the self-hosted backend under `server/`. See [`../../CLAUDE.md`](../../CLAUDE.md)
> for the current layout; the module paths here are kept for reference.
...
├── server/                   # Self-hosted API: Node + Express + PostgreSQL (JWT auth)
├── docker-compose.yml        # Full stack: postgres + api + web (nginx) + backup
├── deploy/nginx/             # nginx config (serves the SPA + reverse-proxies /api)
│
├── components/
...
> Each collection is a **PostgreSQL table** of JSON documents (original IDs preserved from the
> Firebase export). Field shapes below are unchanged from the Firestore era.

### `users/{id}`
```ts
{
  id: string;              // account id (was the Firebase Auth UID)
  name: string;
  email: string;
  phone?: string;
  whatsapp?: string;
...
2. The API (`POST /auth/signup`) creates the account with a bcrypt-hashed password and issues a JWT.
3. System checks for an existing bulk-uploaded profile with the same email → if found, links it to the new account and deletes the old placeholder doc.
...
The service loads every collection from the API up front (via the `firestore-compat.ts` shim over `api-client.ts`) and **polls** for changes every `VITE_POLL_INTERVAL_MS` — there is no `onSnapshot`. Data is stored in private in-memory arrays and exposed via synchronous getter methods for UI performance.

**Key public methods:**

| Method | Description |
|---|---|
| `initialize()` | Restores the JWT session, loads all collections, starts polling |
| `signUp(email, password, userData)` | `POST /auth/signup` — creates the account (bcrypt) + user doc |
| `loginWithPassword(email, password)` | `POST /auth/login` — issues a JWT + enforces PENDING/REJECTED gate |
| `getCurrentUser()` | Returns the profile for the current JWT session (`/auth/me`) |
| `signOut()` | Clears the JWT session |
...
> Enforced **server-side** by the API (`server/src/authz.ts` + `middleware/authenticate.ts`) — a
> port of the original `firestore.rules`. The table below is the effective read/write policy per collection.
...
**Admin check:** enforced in `server/src/authz.ts` — the request's user is admin if their email equals the bootstrap admin (`BOOTSTRAP_ADMIN_EMAIL`) OR their `users` doc has `role == 'ADMIN'`.
...
> [!IMPORTANT]
> Three fields are stored as **JSON strings** inside the document and must be parsed on read:
...
The `persistItem` helper in `store.ts` handles serialization on write; the compat layer deserializes on read. When writing a raw document update, always `JSON.stringify` these fields.
...
3. **All data mutations go through `dataService`** — never call the API directly from page components.

4. **`undefined` values are stripped** before writes (the persistence layer expects clean JSON).

5. **Avatar upload** — images are square-cropped, resized to 200×200px, and stored as a WebP base64 data URL directly in the `users` doc (`avatarUrl` field). No object storage is used.

6. **Notification delivery** — notifications are written to the database (through the API) synchronously after mutations (addAssessment, updateEvidenceStatus, addNomination, etc.). Dynamic notifications (pending approvals, team assessment alerts) are generated client-side in `getNotifications()` without persisting.
