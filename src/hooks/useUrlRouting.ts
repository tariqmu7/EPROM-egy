...
export function useUrlRouting({ isAuthenticated, activeTab, profileUserId, applyRoute }: Options) {
  ...
  useEffect(() => {
    ...
    // If the address bar already resolves to the active tab, leave it alone. This
    // preserves a sub-view segment the page itself manages (e.g. /dashboard/history)
    // which our activeTab-derived path would otherwise flatten to /dashboard —
    // e.g. right after adopting a deep link, when applyRoute changes activeTab.
    const current = pathToRoute(window.location.pathname);
    const sameTab = current?.tab === activeTab;
    const sameProfile =
      activeTab !== 'ceo-view-profile' ||
      (current?.profileUserId ?? null) === (profileUserId ?? null);
    if (sameTab && sameProfile) return;
