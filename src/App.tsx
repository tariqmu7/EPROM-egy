...
const App: React.FC = () => {
  ...
  // Each EvaluationsHub sub-tab has its own route. When the user switches sub-tab
  // inside the hub, point activeTab at that route so the URL reflects it (making
  // the active evaluation refresh-stable + deep-linkable). No tabKey bump → the
  // hub is not remounted; it keeps the sub-tab it already set internally.
  const handleEvalTabChange = useCallback(
    (t: 'online' | 'interviews' | '360' | 'appraisal' | 'evidence') => {
      const key = {
      ...
      <div className="min-h-screen flex bg-slate-50">
        ...
        <div className="hidden lg:flex lg:w-1/2 bg-blue-900 relative overflow-hidden flex-col justify-between p-12">
            <div className="absolute inset-0 z-0">
                <img src={loginBg} alt="Industrial Background" className="w-full h-full object-cover opacity-20" />
  ...
  const renderContent = () => {
    ...
    switch(activeTab) {
        case 'emp-dashboard': return <EmployeeDashboard user={user} routed />;
        ...
        case 'evaluations': return <EvaluationsHub currentUser={user} onTabChange={handleEvalTabChange} />;
        // Sub-tab deep-link cases — open hub with the correct sub-tab pre-selected.
        case 'online-assessments': return <EvaluationsHub currentUser={user} initialTab="online" onTabChange={handleEvalTabChange} />;
        case 'interviews': return <EvaluationsHub currentUser={user} initialTab="interviews" onTabChange={handleEvalTabChange} />;
        case 'emp-assessment': return <EvaluationsHub currentUser={user} initialTab="360" onTabChange={handleEvalTabChange} />;
        case 'emp-appraisal': return <EvaluationsHub currentUser={user} initialTab="appraisal" onTabChange={handleEvalTabChange} />;
        case 'evidence-portal': return <EvaluationsHub currentUser={user} initialTab="evidence" onTabChange={handleEvalTabChange} />;
