...
interface Props {
  ...
  initialTab?: EvalTab;
  /**
   * Notifies the app when the user switches sub-tab, so the URL can update to the
   * sub-tab's own route (/evaluations/360 …). This is what makes the active
   * evaluation survive a refresh + be deep-linkable. Optional so the hub still
   * works standalone.
   */
...
export const EvaluationsHub: React.FC<Props> = ({ currentUser, initialTab = 'online', onTabChange }) => {
  const [activeTab, setActiveTab] = useState<EvalTab>(initialTab);

  const selectTab = (id: EvalTab) => {
    setActiveTab(id);       // instant UI
    onTabChange?.(id);      // let the app sync the URL to this sub-tab's route
  };
