...
// chart splits the BD&Marketing AGM unit into two sections (Business Development
// + Marketing Programs); these combined `bizdev-mkt-snr/jnr/fresh` posts on the
// AGM node `d-bizdev-mkt` are replaced by section-scoped ladders. Kept for ref.
//
// Seed the SP / JP / FR sub-position job profiles that sit UNDER the Business
// Development & Marketing Programs Section Head, in the live `jobProfiles`
// collection.
//
...
// the 13 `sk-*` skills in the `skills` collection. The positions below the SH
// (Senior / Junior / Fresh) are NOT their own org-chart boxes — they live inside
// the same SECTION node `d-bizdev-mkt`. They reuse the SAME 13 skills but at the
...
// already created the `sk-*` skill docs. It only upserts the 3 sub-position
// profiles by deterministic id and never deletes anything.
//
// Auth: reuses the Firebase CLI login (same mechanism as seed-bizdev-mkt-profile.mjs).
//
// Usage:  node scripts/seed-bizdev-mkt-subpositions.mjs [--dry-run]
...
// The 13 skills owned by the Section Head profile, in display order.
const SKILL_IDS = [
  'sk-market-research',
  'sk-bizdev-strategy',
...
// the SH profile is owned by seed-bizdev-mkt-profile.mjs and is NOT written here).
//                          [ SH, SP, JP, FR ]
const LEVELS = {
  'sk-market-research':      [4, 3, 2, 1],
  'sk-bizdev-strategy':      [4, 3, 2, 1],
  'sk-marketing-programs':   [4, 3, 2, 1],
...
const POSITIONS = [
  { id: 'bizdev-mkt-snr',   orgLevel: 'SP', col: 1, title: 'Business Development & Marketing — Senior Position', titleAr: 'تنمية الأعمال وبرامج التسويق — أخصائي أول' },
  { id: 'bizdev-mkt-jnr',   orgLevel: 'JP', col: 2, title: 'Business Development & Marketing — Junior Position', titleAr: 'تنمية الأعمال وبرامج التسويق — أخصائي' },
  { id: 'bizdev-mkt-fresh', orgLevel: 'FR', col: 3, title: 'Business Development & Marketing — Fresh',           titleAr: 'تنمية الأعمال وبرامج التسويق — حديث التعيين' },
];
